...
parser.add_argument('--lq_path', type=str, default='../VOC_dark/train/data', help='Path of the validation dataset')
parser.add_argument('--gt_path', type=str, default='../VOC_dark/train/gt', help='Path of the validation dataset')
parser.add_argument('--an_path', type=str, default='../VOC_dark/train/an', help='Path of the validation dataset')
...
assert opt.model_res.lower() in ['zerodce', 'sci', 'sm']
...
##### log path #####
print(-np.log10(opt.lr))
opt.log_path = opjoin('./log', opt.model_res+"_"+opt.model_det+"_ftdifficult_smbatch_lr{}".format(str(int(-np.log10(opt.lr)))))
...
if opt.model_res.lower() == 'zerodce':
    ...
    model = SCI.Finetunemodel(opt.resume).to(device)
...
#    tf.config.experimental.set_visible_devices(gpus[3], 'GPU')
    ...
    detector = yolov3.YOLOv3_Darknet53_10(weights='pretrained_weights/detect_models/yolov3_10_best.h5', model_img_size=(opt.det_size, opt.det_size))
...
##### class_dict ######
class_dict = ['bicycle', 'boat', 'bottle', 'bus', 'car', 'cat', 'chair', 'dog', 'motorbike', 'person',]
...
gt_files = [f_[:-4]+'.png' for f_ in lq_files]
an_files = [f_[:-4]+'.txt' for f_ in lq_files]
