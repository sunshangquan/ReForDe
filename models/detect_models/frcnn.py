import sys
import os
sys.path.append(os.path.abspath('/home1/ssq/proj3/reforde2/models/detect_models/frcnn_utils/pytorch-faster-rcnn/lib'))
sys.path.append(os.path.abspath('/home1/ssq/proj3/reforde2/models/detect_models/frcnn_utils/pytorch-faster-rcnn/tools'))
from frcnn_utils.utils.creator_tool import AnchorTargetCreator, ProposalTargetCreator
from frcnn_utils.model import FRCNN_VGG16, FRCNN_VGG16_10
...
#    classes = ['person', 'car', 'bus', 'bicycle', 'motorcycle']
    classes = []
    def __init__(self, weights, model_img_size=(600, 600), device="cuda"):
        # load network
    ...
    def detect(self, x, iou_threshold=0.45, conf_threshold=0.70):
        x_ = x.copy()[:, :, :, [2, 1, 0]]
...
#class FRCNN(FRCNN_VGG16):
    def __init__(self, model_img_size=(600, 600), device='cuda', dark=False):
        ...
        self.confidence_thresh_eval = 0.05
        if dark:
            self.faster_rcnn = FRCNN_VGG16_10().to(device)
        else:
            self.faster_rcnn = FRCNN_VGG16().to(device)
    ...
    def load(self, path, load_optimizer=True, parse_opt=False, ):
        print(self.faster_rcnn.classes)
    ...
    def detect(self, x, iou_threshold=0.3, conf_threshold=0.7):
        x_local = x.copy() * 255
    ...
    def compute_object_mislabeling_gradient_and_loss(self, x, detections):
        ...
        return x_tensor.grad.data.cpu().numpy().transpose((0, 2, 3, 1)), losses.object_mislabeling_loss.item()
