...
#        self.vis = Visualizer(env=opt.env)
    ...
    def train_step_(self, x, detections):
        x_local = x.copy() * 255
        ...
        return x_tensor, _bboxes, _labels, _scale

    def train_step(self, x, detections, reverse=False):
        imgs, bboxes, labels, scale = self.train_step_(x, detections)
        self.optimizer.zero_grad()
        losses = self.forward(imgs, bboxes, labels, scale)
        if reverse:
            losses._replace(total_loss = -1 * losses.total_loss)
